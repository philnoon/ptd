...
class MY_Form_validation extends CI_Form_validation
{
	 ...
	 * @return bool
	 */
	function unique($value, $params, $fkey = 'unique', $msg = 'The value in &quot;%s&quot; is already being used.')
	{
		$this->CI->form_validation->set_message($fkey, $msg);
	...
	//--------------------------------------------------------------------
    public function unique_username($value, $params)
    {
        return $this->unique($value, $params, 'unique_username', 'This username is taken.');
    }
    
    public function unique_email($value, $params)
    {
        return $this->unique($value, $params, 'unique_email', 'This email address is taken.');
    }
    
    public function matches_password($str, $field)
    {
        $this->CI->form_validation->set_message('matches_password', 'Password was entered incorrectly');
        $result = TRUE;
    ...
    public function check_old_password($str)
    {
        ...
        $user = $this->CI->user_model->find($this->CI->input->post('id'));
        if(!empty($user))
        {
            if (do_hash($user->salt . $str) == $user->password_hash)
            {
                return TRUE;
            }
        }   
        
        $this->CI->form_validation->set_message('check_old_password', 'The old password is incorrect.');
        return FALSE; 
    }
