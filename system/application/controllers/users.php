...
class Users extends Front_Controller
{
	...
	public function profile()
	{
		...
		if ($this->current_user->user_type === USER_TRAINER && $verified != 'verified'){
		...
		if ($this->current_user->user_type === USER_TRAINER && $this->current_user->trainer_status_paid != 'Y'){
		Template::set_message('You need to pay the annual membership fee before you can quote on training requests.', 'danger');
		}
		
		if($this->current_user->user_type === USER_TRAINER && $this->current_user->trainer_status_paid != 'Y' && $verified != 'verified'){
		Template::set_message('You need to upload and have all your certifications verified before you can quote on training requests.<br />You need to pay the annual membership fee before you can quote on training requests.', 'danger');      
		}  
        ...
        else if($this->input->post('delete_photo'))
        {
            ...
            Template::set_message('You profile photo has been removed.', 'success');
            redirect('profile');
        }
        //end if
		...
		{		    
		    $pagecontents = $this->pages_model->find_all_by('id',16); 
		    Template::set('pagecontents', $pagecontents); 		
		}
		
		if($this->current_user->user_type === USER_MEMBER)
		{		    
		    $pagecontents = $this->pages_model->find_all_by('id',17); 
		    Template::set('pagecontents', $pagecontents); 		
		}	
		...
		/**
		 * Allows a user to see their own account information.
		 *
		 * @access public
		 *
		 * @return void
		...
		public function account()
		{
			...
			//change to update user settings - or get the ratings and purchase history
			//this account area is to pay a yearly fee
			...
			if ($this->input->post('submit'))
			{
				...
				if ($this->_save_profile($user_id))
				{
					Template::set_message(lang('us_account_updated_success'), 'success');
				...
				{
					Template::set_message(lang('us_account_updated_error'), 'danger');
				}//end if
			}
	        ...
	        //do some control here and calculate if its expired
	        
	        
	        //get the current balance
	        //$currentbalance = $this->current_user->trainer_fee_paid;
	
			// get the current user information
			$user = $this->user_model->find($this->current_user->id);
	        Template::set('user', $user);
	
			Template::set_view('users/account');
			Template::render();
	...
	*/
    ...
    {        
        ...
        if($this->current_user === USER_TRAINER)
        {
            ...
            $this->form_validation->set_rules('about_me', 'about me', 'trim|max_length[490]|xss_clean');
            
            $this->form_validation->set_rules('facebook_page', 'facebook page', 'trim|max_length[140]|xss_clean');
            $this->form_validation->set_rules('twitter_page', 'twitter page', 'trim|max_length[140]|xss_clean');
            $this->form_validation->set_rules('youtube_page', 'youtube page', 'trim|max_length[140]|xss_clean');
            $this->form_validation->set_rules('googleplus_page', 'google+ page', 'trim|max_length[140]|xss_clean');
            $this->form_validation->set_rules('website_page', 'wensite page', 'trim|max_length[140]|xss_clean');
        }
        ...
        $data = array(
            ...
            'googleplus_page'=> $this->input->post('googleplus_page'),
            'website_page'  => $this->input->post('website_page'),
        );
    ...
    private function _register_process($user_type=USER_MEMBER)
    {
        ...
        if ($this->input->post('submit'))
        {
            ...
            $this->form_validation->set_rules('full_name', 'lang:bf_full_name', 'trim|strip_tags|max_length[255]|xss_clean');
            $this->form_validation->set_rules('accept', 'Agree to the Terms and Conditions', 'required|isset');
    ...
    public function faq()
        {
            ...
            $pagecontents = $this->pages_model->find_all_by('id',15); 
            Template::set('pagecontents', $pagecontents);
            Template::set_view('users/faq');
            Template::set('page_title', 'FAQ');
            Template::render();
...
public function questionnaire()
    {
        ...
        if ($this->input->post('submit'))
        {
            ...
            {
                Template::set_message('Questionnaire successfully updated', 'success');
            ...
            {
                Template::set_message('There was an error', 'danger');
            }//end if
        }//end if
    ...
    private function _save_questionnaire($id)
        {
            ...
            if ( $_POST['id'] != $this->current_user->id )
            {
                $this->form_validation->set_message('exc_freq', 'lang:us_invalid_userid');
                return FALSE;
            }
    
            $this->form_validation->set_rules('exc_freq', 'lang:bf_password', 'required');
    		$this->form_validation->set_rules('fitness_rating', 'lang:bf_password', 'required');
            ...
            $data = array(
                ...
                'exc_freq' => $this->input->post('exc_freq'),
    			'fitness_rating' => $this->input->post('fitness_rating'),
    			'had_stroke' => $this->input->post('had_stroke'),
    			'chest_pain' => $this->input->post('chest_pain'),
				'dizziness' => $this->input->post('dizziness'),
    			'asthma' => $this->input->post('asthma'),
    			'diabetes' => $this->input->post('diabetes'),
    			'bodyproblems' => $this->input->post('bodyproblems'),
    			'dangerous' => $this->input->post('dangerous')
            );
