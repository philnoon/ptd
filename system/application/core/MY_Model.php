	...
	*/
	...
	protected function _function_check($id=FALSE, &$data=FALSE)
	{
		...
		if (empty($this->table))
		{
			...
			$this->logit('['. get_class($this) .': '. __METHOD__ .'] '. $this->lang->line('bf_model_no_table'), 'danger');
			return FALSE;
		}
	...
	//--------------------------------------------------------------------
    public function set_alias($alias='t1')
    {
        if($this->has_alias) return;
